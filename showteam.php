...
$query = $db->simple_select('usergroups', 'gid, title, usertitle', 'showforumteam=1', ['order_by' => 'disporder']);
...
if ($mybb->settings['showgroupleaders']) {
    ...
    if (! empty($leaders)) {
        ...
        $usergroups[0] = ['gid' => 0, 'title' => $lang->group_leaders, 'usertitle' => $lang->group_leaders];
...
$query = $db->simple_select('users', 'uid, username, displaygroup, usergroup, additionalgroups, ignorelist, hideemail, receivepms, lastactive, lastvisit, invisible, away', $query_part."displaygroup IN ($groups_in) OR (displaygroup='0' AND usergroup IN ($groups_in)) OR uid IN ($users_in)", ['order_by' => 'username']);
...
foreach ($usergroups as $usergroup) {
    ...
    foreach ($usergroup['user_list'] as $user) {
        ...
        if ($user['lastactive'] > $timecut && ($user['invisible'] == 0 || $mybb->usergroup['canviewwolinvis'] == 1) && $user['lastvisit'] != $user['lastactive']) {
            $status = $lang->postbit_online;
        } elseif ($user['away'] == 1 && $mybb->settings['allowaway'] != 0) {
            $status = $lang->postbit_status_away;
        } else {
            $status = $lang->postbit_offline;
        }

        if ($user['invisible'] == 1 && $mybb->usergroup['canviewwolinvis'] != 1 && $user['uid'] != $mybb->user['uid']) {
            $user['lastvisit'] = $lang->lastvisit_hidden;
        } elseif ($user['lastactive']) {
