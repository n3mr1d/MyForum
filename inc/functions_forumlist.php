        ...
        foreach ($parent as $forum) {
            ...
            if (isset($permissions['canonlyviewownthreads']) && $permissions['canonlyviewownthreads'] == 1) {
                ...
                if (! is_array($private_forums)) {
                    ...
                    if (! empty($fids)) {
                        $fids = implode(',', $fids);
                        $query = $db->simple_select('threads', 'tid, fid, subject, lastpost, lastposter, lastposteruid', "uid = '{$mybb->user['uid']}' AND fid IN ({$fids}) AND visible != '-2'", ['order_by' => 'lastpost', 'order_dir' => 'desc']);
            ...
            if ($forum['linkto'] == '') {
                ...
                if ($lastpost_data['lastpost'] == 0 && $hideinfo != true) {
                    ...
                    $lastpost_data['lastposter'] = format_name($lastpost_data['lastposter'], $lastpost_data['lastposterusergroup'], $lastpost_data['lastposterdsgroup']);
...
function get_forum_lightbulb($forum, $lastpost, $locked = 0)
{
    ...
    elseif ($forum['open'] == 0 || $locked) {
        ...
        if (! empty($forum['lastread'])) {
            ...
            } elseif ($mybb->user['uid'] && $mybb->settings['threadreadcut'] > 0 && $threadcut > $lastpost['lastpost']) {
