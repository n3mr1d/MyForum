...
public function __construct()
{
    ...
    $plugins->hooks["pre_output_page"][10]["styleUsernames_parseUsernames"] = [
        "function" => function (&$arg) use ($plugins) {
            $plugins->objects['styleUsernames']->parseUsernames($arg);
        }
    ];

    $plugins->hooks["global_end"][10]["styleUsernames_getModerators"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->getModerators();
        }
    ];

    $plugins->hooks["build_forumbits_forum"][10]["styleUsernames_buildForumbits"] = [
        "function" => function (&$arg) use ($plugins) {
            $plugins->objects['styleUsernames']->buildForumbits($arg);
        }
    ];

    $plugins->hooks["forumdisplay_announcement"][10]["styleUsernames_forumdisplayAnnouncement"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->forumdisplayAnnouncement();
        }
    ];

    $plugins->hooks["forumdisplay_thread"][10]["styleUsernames_forumdisplayThread"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->forumdisplayThread();
        }
    ];

    $plugins->hooks["search_results_thread"][10]["styleUsernames_searchThread"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->searchThread();
        }
    ];

    $plugins->hooks["search_results_post"][10]["styleUsernames_searchPost"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->searchPost();
        }
    ];

    $plugins->hooks["private_message"][10]["styleUsernames_privateMessage"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->privateMessage();
        }
    ];

    $plugins->hooks["portal_announcement"][10]["styleUsernames_portalAnnouncement"] = [
        "function" => function () use ($plugins) {
            $plugins->objects['styleUsernames']->portalAnnouncement();
        }
    ];

    $plugins->hooks["pre_output_page"][10]["styleUsernames_pluginThanks"] = [
        "function" => function (&$arg) use ($plugins) {
