...
function socialsites_install()
{
    ...
    foreach ($sites as $site) {
        $insert = [
            ...
            'icon' => $db->escape_string($site['icon']),
            'url' => $db->escape_string($site['url']),
            'color' => $db->escape_string($site['color']),
...
function socialsites_register()
{
    ...
    $lang->load('socialsites');
    $query = $db->simple_select('socialsites', '*', '', ['order_by' => 'nicename', 'order_dir' => 'ASC']);
    $social_sites = [];
...
function socialsites_admin_config_menu($sub_menu)
{
    ...
    $lang->load('config_socialsites');
    $sub_menu[] = ['id' => 'socialsites', 'title' => $lang->socialsites, 'link' => 'index.php?module=config-socialsites'];
...
function socialsites_admin_load()
{
    ...
    if ($page->active_action == 'socialsites') {
        ...
        if ($mybb->input['action'] == 'do_add' || $mybb->input['action'] == 'do_edit') {
            ...
            foreach (['image_large', 'image_small'] as $size) {
                if ($_FILES[$size]['name']) {
            ...
            $form_container->output_row(
                'Font Awesome Icon <em>*</em>',
                'Enter a Font Awesome class, e.g. <code>fa-brands fa-facebook</code> or <code>fa-solid fa-globe</code>.',
                $form->generate_text_box('icon', $site['icon'], ['id' => 'icon', 'style' => 'width: 250px;'])
            );
            $form_container->output_row(
                'Color For Icon <em>*</em>',
                $form->generate_text_box('color', $site['color'], ['id' => 'color', 'style' => 'width:240px'])
            );
            $form_container->end();
            ...
            if (! empty($sites)) {
                ...
                $table->construct_header($lang->socialsites_icon, ['width' => '10%', 'class' => 'align_center']);
                $table->construct_header($lang->socialsites_site, ['width' => '20%', 'class' => 'align_center']);
                $table->construct_header($lang->socialsites_details, ['width' => '40%', 'class' => 'align_center']);
                $table->construct_header($lang->controls, ['width' => '30%', 'colspan' => 2, 'class' => 'align_center']);

                foreach ($sites as $site) {
                    $table->construct_cell("<img src=\"{$mybb->settings['bburl']}/images/socialicons/{$site['image_large']}\" alt=\"{$site['name']}\" title=\"{$site['title']}\" />", ['class' => 'align_center']);
                    $table->construct_cell($site['name'], ['class' => 'align_center']);
                    $table->construct_cell($site['url'], ['class' => 'align_center']);
                    $table->construct_cell("<a href=\"index.php?module=config-socialsites&amp;action=edit&amp;ssid={$site['ssid']}&amp;my_post_key={$mybb->post_code}\">{$lang->edit}</a>", ['class' => 'align_center', 'width' => '15%']);
                    $table->construct_cell("<a href=\"index.php?module=config-socialsites&amp;action=delete&amp;ssid={$site['ssid']}\">{$lang->delete}</a>", ['class' => 'align_center', 'width' => '15%']);
                    $table->construct_row();
                }
