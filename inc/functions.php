	...
	if (is_array($pforumcache[$fid])) {
		foreach ($pforumcache[$fid] as $key => $forumnav) {
			if ($fid == $forumnav['fid']) {
				...
				// tambahkan kolom icon dari database
				if (!empty($forumnav['icon'])) {
					$navbits[$navsize]['icon'] = $forumnav['icon'];
				} else {
					$navbits[$navsize]['icon'] = "<i class='fa-regular fa-folder'></i>"; // default kalau kosong
				}
