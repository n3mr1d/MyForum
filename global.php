...
if (isset($mybb->input['language']) && $lang->language_exists($mybb->get_input('language')) && verify_post_check($mybb->get_input('my_post_key'), true)) {
    ...
    if ($mybb->user['uid']) {
        ...
        $db->update_query('users', ['language' => $db->escape_string($mybb->settings['bblanguage'])], "uid = '{$mybb->user['uid']}'");
    }
    // Guest = cookie
    else {
...
if (in_array($current_page, $valid)) {
    ...
    if (isset($mybb->input['pid']) && THIS_SCRIPT != 'polls.php') {
        $query = $db->simple_select('posts', 'fid', "pid = '{$mybb->input['pid']}'", ['limit' => 1]);
    ...
    elseif (isset($mybb->input['tid'])) {
        $query = $db->simple_select('threads', 'fid', "tid = '{$mybb->input['tid']}'", ['limit' => 1]);
...
if ($loadstyle != "def='1'") {
    $query = $db->simple_select('themes', 'name, tid, properties, stylesheets, allowedgroups', $loadstyle, ['limit' => 1]);
    $theme = $db->fetch_array($query);

    if ($theme && ! $load_from_forum && ! is_member($theme['allowedgroups']) && $theme['allowedgroups'] != 'all') {
        if ($load_from_user == 1) {
            $db->update_query('users', ['style' => 0], "style='{$mybb->user['style']}' AND uid='{$mybb->user['uid']}'");
        }
...
$templatelist .= ' ,header_navigation_bar,header_welcomeblock_member_buddy,header_welcomeblock_member_pms,header_welcomeblock_member_search,header_welcomeblock_guest,header_welcomeblock_guest_login_modal,header_welcomeblock_guest_login_modal_lockout';
...
if ($mybb->user['uid'] != 0) {
    $useravatar = format_avatar($mybb->user['avatar'], $mybb->user['avatardimensions'], '40x40');
...
eval('$navigation_bar ="'.$templates->get('header_navigation_bar').'";');
...
if ($mybb->user['uid'] && is_banned_email($mybb->user['email']) && $mybb->settings['emailkeep'] != 1) {
    if (
        ! (THIS_SCRIPT == 'usercp.php' && in_array($mybb->get_input('action'), ['email', 'do_email'])) &&
